...
function init_kuscia_conf_file() {
  ...
  if [[ "${domain_type}" = "lite" ]]; then
    ...
    docker run -it --rm ${KUSCIA_IMAGE} kuscia init --mode "${domain_type}" --domain "${domain_id}" -r "${runtime}" --master-endpoint ${master_endpoint} --lite-deploy-token ${token} | tr -d '\000' > "${kuscia_conf_file}" 2>&1 || cat "${kuscia_conf_file}"
  else
    docker run -it --rm ${KUSCIA_IMAGE} kuscia init --mode "${domain_type}" --domain "${domain_id}" -r "${runtime}" | tr -d '\000' > "${kuscia_conf_file}" 2>&1 || cat "${kuscia_conf_file}"
  fi
